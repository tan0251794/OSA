...
class Order(models.Model):
    ...
    order_no = models.CharField(max_length=50, editable=False, blank=True, unique=True)
    ...
    def save(self, flag=True, *args, **kwargs):
        ...
        if flag:        
            self.order_no = self.order_no = str(dt.now().strftime(
...
class Product(models.Model):
    ...
    pack = models.ForeignKey("purchase_app.Pack", on_delete=models.DO_NOTHING, null=True)
...
class Pack(models.Model):
    ...
    oversea_ship_fee = models.DecimalField( max_digits=12, decimal_places=2, default=0)
    foreign_domestic_fee = models.DecimalField(max_digits=12, decimal_places=2, default=0)
    vn_domestic_fee = models.DecimalField(max_digits=12, decimal_places=2, default=0)
    status = models.CharField(
        max_length = 100,
        choices=STATUS_CHOICES,
        default='REGISTER',
